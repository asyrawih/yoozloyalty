    ...
    public function run()
    {
        ...
        if ($user_count > 0) {
          foreach (range($first, $user_count - 1) as $index) {
            ...
            if ($gender == 'male') {
              ...
              $avatar = base_path() . '/database/seeders/avatars/men/' . $men . '.jpg';
              $men++;
            } else {
              $firstName = $faker->firstNameFemale;
              $avatar = base_path() . '/database/seeders/avatars/women/' . $women . '.jpg';
              $women++;
            }
        ...
        if ($rewards_count > 0) {
          foreach (range(1, $rewards_count) as $index) {
            ...
            // Images
            $image = database_path('seeders/visuals-' . $image_dir . '/rewards/' . mt_rand(1, 18) . '.jpg');
            $reward
              ->addMedia($image)
              ->preservingOriginal()
          ...
          foreach (range(1, $campaign_count) as $index) {
            ...
            $campaign->content = [
              ...
              "rewardsHeaderContent" => "<p>Earn points and choose from these rewards.</p>",
                "storesHeaderTitle" => "Stores",
                "storesHeaderContent" => "<p>You can find our other stores below.</p>"
            ];
            ...
            // Images
            $image = database_path('seeders/visuals-' . $image_dir . '/home/' . $index . '.jpg');
            $campaign
              ->addMedia($image)
              ->preservingOriginal()
              ->sanitizingFileName(fn($fileName) => strtolower(str_replace(['#', '/', '\\', ' '], '-', $fileName)))
              ->toMediaCollection('home_image');

            $content_images = range(1, 10);
            $site_images = Arr::random($content_images, 5);

            $image = database_path('seeders/visuals-' . $image_dir . '/content/' . $content_images[0] . '.jpg');
            $campaign
              ->addMedia($image)
              ->preservingOriginal()
              ->sanitizingFileName(fn($fileName) => strtolower(str_replace(['#', '/', '\\', ' '], '-', $fileName)))
              ->toMediaCollection('home_item1_image');

            $image = database_path('seeders/visuals-' . $image_dir . '/content/' . $content_images[1] . '.jpg');
            $campaign
              ->addMedia($image)
              ->preservingOriginal()
              ->sanitizingFileName(fn($fileName) => strtolower(str_replace(['#', '/', '\\', ' '], '-', $fileName)))
              ->toMediaCollection('home_item2_image');

            $image = database_path('seeders/visuals-' . $image_dir . '/content/' . $content_images[2] . '.jpg');
            $campaign
              ->addMedia($image)
              ->preservingOriginal()
              ->sanitizingFileName(fn($fileName) => strtolower(str_replace(['#', '/', '\\', ' '], '-', $fileName)))
              ->toMediaCollection('home_item3_image');

            $image = database_path('seeders/visuals-' . $image_dir . '/content/' . $content_images[3] . '.jpg');
            $campaign
              ->addMedia($image)
              ->preservingOriginal()
              ->sanitizingFileName(fn($fileName) => strtolower(str_replace(['#', '/', '\\', ' '], '-', $fileName)))
              ->toMediaCollection('earn_header_image');

            $image = database_path('seeders/visuals-' . $image_dir . '/content/' . $content_images[4] . '.jpg');
            $campaign
              ->addMedia($image)
              ->preservingOriginal()
              ->sanitizingFileName(fn($fileName) => strtolower(str_replace(['#', '/', '\\', ' '], '-', $fileName)))
              ->toMediaCollection('rewards_header_image');

            $image = database_path('seeders/visuals-' . $image_dir . '/content/' . $content_images[5] . '.jpg');
            $campaign
              ->addMedia($image)
              ->preservingOriginal()
              ->sanitizingFileName(fn($fileName) => strtolower(str_replace(['#', '/', '\\', ' '], '-', $fileName)))
              ->toMediaCollection('contact_header_image');
