<template>
    <v-container> <div v-if="loadingForm" class="px-4 py-3" >
            <v-progress-linear
                                    ...
                                    <!-- for subs menu includes in here -->
                                        ...
                                        <div v-for="(form_item, index) in sub.items" :key="index">
                                                ...
                                                <div v-if="form_item.type == 'password'">
                                                    <v-text-field
                                                        :type="
                                                            formModel[ form_item.column + '_show' ] ? 'text' : 'password'
                                                        "
                                                        :append-icon="
                                                            formModel[
...
import CoupunList from "../../../components/ui/DataFormComponents/CoupunList"
import CoupunHistory from "../../../components/ui/DataFormComponents/CoupunHistory"
...
export default {
    ...
    async beforeMount(){
        const { form } = await this.getDataFromApi()
            ...
            if (this.mode === "crud") {
                ...
                for (let date of dates) {
                    if (this.formModel[date] !== null) {
                        this.formModel[date] = new Date(this.formModel[date])
                    }
                }
            }
        }
    ...
    methods: {
        ...
        getRelatedData(column, relation) {
            axios
                ...
                .then(res => {
                    if (res.data.status === "success") {
                        this.formModel[column + "_items"] = res.data.fields;
                        this.formModel[column + "_loading"] = false;
                    }
                })
