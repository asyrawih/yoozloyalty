<template>
    <v-card>
        <v-toolbar tabs flat>
            <v-toolbar-title>Store Setup</v-toolbar-title>
            <v-spacer></v-spacer>
                ...
                >
                    <v-tab-item key="general" :value="'general'" :eager="true">
                        <v-text-field
                            ...
                            class="mb-3"
                            hint="The brand name is the title that is visible in the top bar."
                            persistent-hint
                        ></v-text-field>
                        <v-text-field
                            ...
                            :error-messages="
                                errors.collect('form1.app_contact')
                            "
                    ...
                    <v-tab-item key="email" :value="'email'" :eager="true">
                        <v-text-field
                            ...
                            label="Brand e-mail name"
                            :error-messages="
                                errors.collect('form1.app_mail_name_from')
                            "
                            v-validate="'required|min:3|max:64'"
                            class="mb-3"
                        ...
                        <v-text-field
                            ...
                            label="Brand e-mail address"
                            :error-messages="
                                errors.collect('form1.app_mail_address_from')
                            "
                            v-validate="'required|max:64|email'"
                            class="mb-3"
                    ...
                    <v-tab-item key="domain" :value="'domain'" :eager="true">
                        <p class="subtitle-2">
                            Be careful when changing this domain. The website
                            will become unusable on the current domain, and only
                            become visible on the new domain if the DNS settings
                            are correct.
                        </p>
                        ...
                        <p class="subtitle-2">
                            DNS changes may take a couple of hours to propagate.
                        </p>
                    </v-tab-item>
                </v-tabs-items>
            </v-card-text>
...
export default {
    ...
    created() {
        axios
            .get("/admin/store", { params: { locale: this.$i18n.locale } })
            .then(response => {
                this.form1.app_name = response.data.app_name;
                this.form1.app_contact = response.data.app_contact;
                this.form1.app_mail_name_from =
                    response.data.app_mail_name_from;
                this.form1.app_mail_address_from =
                    response.data.app_mail_address_from;
                this.form1.app_name = response.data.app_name;
