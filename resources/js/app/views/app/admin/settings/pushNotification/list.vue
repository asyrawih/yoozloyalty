      ...
      <v-card flat>
        <v-card-title>
          ...
          <v-btn
            ...
            @click="smsTestDialog = !smsTestDialog"
          >
            SMS Test
        ...
        <v-row justify="center">
          <v-dialog v-model="smsTestDialog" persistent max-width="600px">
            <v-form ref="formSmsTest" lazy-validation>
              <v-card>
                <v-card-title>
                  <span class="text-h5">SMS Test</span>
                </v-card-title>
                <v-card-text>
                  <v-container>
                    <v-row>
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="darken-1" text @click="smsTestDialog = false">
                    Close
                  </v-btn>
                  <v-btn color="blue darken-1" text @click="sendSmsTest">
                    Send
                  </v-btn>
                </v-card-actions>
              </v-card>
            ...
            <v-expansion-panel-content>
              <v-form ref="filterForm">
                <v-row>
                  <v-col cols="12" sm="4">
                    <v-text-field
                      v-model="filter.isd_code"
                      label="ISD Code"
                    ></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="4" class="p-0">
                    <v-text-field
                      v-model="filter.sms_services"
                      label="SMS Service"
                    ></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="4" class="p-0">
  ...
  methods: {
    ...
    getDataFromApi() {
      const params = {
        ...
        isd_code: this.filter.isd_code,
        sms_services: this.filter.sms_services,
        status: this.filter.status,
      };
      axios
        .get("/admin/sms", { params: params })
        .then((r) => {
    ...
    getListCountry() {
      axios
        .get("/localization/full-country")
        .then((r) => {
          this.listCountry = r.data.data || [];
        })
        .catch((err) => {
          console.log(err);
        });
    ...
    save() {
      ...
      if (this.$refs.form.validate() === true) {
        let params = {
          ...
          isd_code: this.editedItem.isd_code,
          status: this.editedItem.status,
          scheme: this.editedItem.scheme,
        };
        axios
          .post("/admin/sms", params)
          .then((r) => {
    ...
    sendSmsTest() {
      this.$refs.formSmsTest.validate();
      if (this.$refs.formSmsTest.validate() === true) {
        let params = {
          isd_code: this.smsTest.isd_code,
          phone_number: this.smsTest.phone_number,
        };
        axios
          .post("/admin/sms/test", params)
          .then((r) => {
            this.$root.$snackbar(r.data.message);
            this.smsTestDialog = false;
            this.getDataFromApi();
            this.$refs.formSmsTest.reset();
          })
          .catch((err) => {
            this.$root.$snackbar(err.response.data.message);
            this.smsTestDialog = false;
    ...
    editItem(item) {
      this.editedItem = {
        ...
        isd_code: item.country_isd_code,
    ...
    processEdit() {
      ...
      if (this.$refs.form.validate() === true) {
        ...
        axios
          .put(`/admin/sms/${this.editedItem.id}`, params)
          .then((r) => {
            this.$root.$snackbar(r.data.message);
            this.dialog = false;
            this.getDataFromApi();
