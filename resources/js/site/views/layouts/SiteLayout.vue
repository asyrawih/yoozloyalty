<template>
  <v-app :style="{ 'background-color': website.theme.backgroundColor }">
    <v-navigation-drawer
      ...
      right
      :style="{
        'background-color': website.theme.drawer.backgroundColor,
        color: website.theme.drawer.textColor,
      }"
    >
      <v-flex
      ...
      <v-flex xs12 align-center justify-center layout text-xs-center>
        ...
        >
          <div :class="{ 'mt-5': !website.theme.logo }" class="title">
            {{ account.app_name }}
          </div>
        </router-link>
      </v-flex>
                ...
                :style="{
                  color:
                    item.route_name === $route.name
                      ? highlighted.color
                      : website.theme.drawer.textColor,
                }"
                class="ml-4 nav-icon"
                >{{ item.icon }}</v-icon
              >
    ...
    >
      <router-link
        ...
        :to="{ name: 'home' }"
        tag="img"
        :src="website.theme.logo"
        style="max-height: 70%; max-width: 240px; cursor: pointer"
        class="ml-2 mr-4"
        :alt="account.app_name"
      ></router-link>
      <v-toolbar-title
        class="mr-5 align-center"
        v-if="account.app_headline || !website.theme.logo"
      >
        <v-spacer></v-spacer>
        <span class="title"
          ><router-link
            :to="{ name: 'home' }"
            :style="{
              color: website.theme.primaryTextColor,
              'text-decoration': 'none',
            }"
            >{{ account.app_name }}</router-link
          ></span
        >
        <div class="subtitle-2" v-if="account.app_headline">
          {{ account.app_headline }}
        </div>
      </v-toolbar-title>
      <v-spacer></v-spacer>
      <v-toolbar-items>
    ...
    >
      <v-card class="flex" text tile>
        <v-card-actions
          ...
          :style="{
            'background-color': website.theme.primaryColor,
            color: website.theme.primaryTextColor,
          }"
        >
          ...
          >
          <!--          <span v-html="account.app_contact"></span>-->
        </v-card-actions>
      </v-card>
    </v-footer>
    ...
    >
      <v-row>
        ...
        <v-col cols="3" md="1" align-self="center">
          <v-btn color="white" text :to="{ name: 'legal' }"> Terms </v-btn>
        </v-col>
...
export default {
  ...
  mounted() {
    ...
    let appBarContent = document.querySelector(
  ...
  computed: {
    ...
    highlighted() {
      return {
        "background-color":
          this.$store.state.app.website.theme.drawer.highlightBackgroundColor,
        color: this.$store.state.app.website.theme.drawer.highlightTextColor,
      };
    },
  },
};
