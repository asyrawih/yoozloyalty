    ...
    <v-app :style="{ 'background-color': backgroundColor }">
        ...
        >
            ...
            <v-menu
                ...
                left
                nudge-bottom="60px"
                v-if="$vuetify.breakpoint.mdAndUp && $auth.check()"
                class="ml-3"
            >
        ...
        <v-main
            :style="{
                ...
                'background-color': campaign.theme.backgroundColor
            }"
        >
        ...
        >
            <v-card class="flex" text tile>
                ...
                >
                    &copy;{{ new Date().getFullYear() }} — {{ campaign.business.name }}

                    <span class="mx-2">&bull;</span>

                    <span :style="{ color: campaign.theme.primaryTextColor }">
                        Staff Area
                    </span>
                </v-card-actions>
            </v-card>
...
export default {
    ...
    created: function() {
        ...
        if (
            this.$auth.check() &&
            typeof this.$auth.user().language !== "undefined" &&
            this.$auth.user().language !== "undefined"
        ) {
    ...
    computed: {
        ...
        topNav() {
            if (this.$auth.check()) {
                ...
                if (this.$can('credit-request')) {
                    topNav.push({
                        name: "Credit Request",
                        icon: "request_page",
                        route_name: "credit-request",
                        to: { name: "credit-request" }
                    });
                }
        ...
        campaign() {
            return this.app.campaign;
        },
        highlighted() {
            return {
                "background-color": this.app.campaign.theme.drawer
                    .highlightBackgroundColor,
                "color": this.app.campaign.theme.drawer
                    .highlightTextColor
            };
        }
    }
