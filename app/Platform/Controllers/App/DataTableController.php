...
class DataTableController extends \App\Http\Controllers\Controller
{
    ...
    */
    ...
    public function getDataList(Request $request)
    {
        ...
        $extraActions = method_exists($model, 'getExtraActions') ? $model::getExtraActions()[auth()->user()->role] : [];
        $allHeaders = $model::getHeaders()[$user->role];
        $extraSelectColumns = method_exists($model, 'getExtraSelectColumns') ? $model::getExtraSelectColumns()[$user->role] : [];
        $extraQueryColumns = method_exists($model, 'getExtraQueryColumns') ? $model::getExtraQueryColumns()[$user->role] : [];
        $extraWithQueries = method_exists($model, 'getExtraWithQueries') ? $model::getExtraWithQueries()[$user->role] : [];
        $tableFilters = method_exists($model, 'getTableFilters') ? $model::getTableFilters()[$user->role] : [];
        $exportClass = method_exists($model, 'getExportClass') ? $model::getExportClass()[$user->role] : null;
        $searchColumns = $model::getSearchColumns()[$user->role];
        ...
        */
        ...
        $records = $records->map(function ($item) use ($date_times, $dates, $times, $user) {
            foreach ($date_times as $date_time) {
                if (isset($item[$date_time]) && $item[$date_time] != null) {
                    $value = Carbon::parse($item[$date_time], config('app.timezone'))->setTimezone($user->getTimezone());
            ...
            foreach ($dates as $date) {
                if (isset($item[$date]) && $item[$date] != null) {
                    $value = Carbon::parse($item[$date], config('app.timezone'))->setTimezone($user->getTimezone());
            ...
            foreach ($times as $time) {
                if (isset($item[$time]) && $item[$time] != null) {
                    $value = Carbon::parse($item[$time], config('app.timezone'))->setTimezone($user->getTimezone());
        ...
        $numberFormatter = new NumberFormatter($numberFormatRepository, ['locale' => $user->getLocale()]);
    ...
    public function postDeleteRecords(Request $request)
    {
        ...
        if ($uuids !== null && is_array($uuids)) {
            foreach ($uuids as $uuid) {
                ...
                if (! $query) {
                    ...
                    $merchants[] = $query->name;
                    $erasers[] = $query->deleted_by;
        ...
        if (! $all_records_success) {
            ...
            if ($user->role === 1) {
                ...
                $this->sendNotif(
                    "Merchant deletion approved",
                    "{$user->name} approves the merchant deletion request.",
                    $users,
                    'adminNotif'
                );
            } else if ($user->role === 2) {
                $masterAdminUsers = User::withoutGlobalScopes()
                    ->where('role', 1)
                    ->get(['id', 'uuid']);
                ...
                $this->sendNotif(
                    ...
                    "{$user->name} wants to delete {$merchants} account, please confirm that.",
                    $masterAdminUsers,
                    'adminNotif'
                );
            }
     ...
     * @return \Symfony\Component\HttpFoundation\Response
     */
    public function postRestoreRecords(Request $request)
    {
        $model = $request->model;
        ...
        if (! $records_success) {
            return response()->json([
                'status' => 'error',
                'msg' => 'Records couldn\'t be restored.'
            ], 422);
        } else {
            $users = User::withoutGlobalScopes()
                ->whereIn('id', $erasers)
            ...
            $this->sendNotif(
                "Merchant deletion rejects",
                "{$user->name} rejects the merchant deletion request.",
                $users,
                'adminNotif'
            );
