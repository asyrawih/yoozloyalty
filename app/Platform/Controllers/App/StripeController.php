 ...
 * @group Stripe
 *
 * Endpoint to check user's stripe subscription status
 *
...
class StripeController extends \App\Http\Controllers\Controller {
  ...
  public function postToken(Request $request) {
    ...
    if ($token != null && auth()->check()) {
      ...
      if (! empty($user) && ! empty($plan)) {
        ...
        if ($user->remote_customer_id === null || $user->previous_remote_gateway !== 'stripe') {
          if ($user->previous_remote_customer_id !== null && $user->previous_remote_gateway === 'stripe') {
            $stripe_customer_id = $user->previous_remote_customer_id;
            $user->previous_remote_gateway = 'stripe';
        ...
        if ($stripe_plan_id === null) { // Lifetime
          ...
          if (isset($subscription->plan)) {
            ...
            $user->plan_id = $plan_id;
            $user->remote_customer_id = $subscription->id;
  ...
  public function postWebhook() {
    ...
    if ($event_json->type == 'customer.deleted') {
      ...
      if (! empty($user)) {
        ...
        $body_top .= "Stripe Subcription ID: " . $stripe_customer_id . PHP_EOL . PHP_EOL;
        $body_bottom = "";
  ...
  public function postCancelSubscription() {
    ...
    if ($user->remote_customer_id !== null) {
      ...
      $subscription = \Stripe\Subscription::retrieve($user->remote_customer_id);
      $subscription->cancel();
      $user->previous_remote_customer_id = $user->remote_customer_id;
      $user->remote_customer_id = null;
      $user->save();
      ...
      $body_top .= "Plan: " . $plan->name . PHP_EOL . PHP_EOL;
      $body_top .= "User ID: " . $user->id . PHP_EOL . PHP_EOL;
      $body_top .= "Stripe Subcription ID: " . $user->remote_customer_id . PHP_EOL . PHP_EOL;
      $body_bottom = "";
