...
{
    public function callback($status, Request $request) {
        $orderId = explode('_', $request->txnid)[0];
        ...
        try {
            ...
            if ($invoice) {
                ...
                if ($status === 'success') {
                    ...
                    $invoice->amount_paid = floatval($request->amount);
                    $invoice->paid_at = Carbon::parse($request->addedon)->format('Y-m-d');
                    $invoice->save();
